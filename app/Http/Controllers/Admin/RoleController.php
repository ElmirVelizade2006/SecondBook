...
class RoleController extends Controller
{
    ...
    {
        $roles = Role::query()->withCount(['users', 'permissions'])->orderByDesc('is_system')->orderBy('display_name')->paginate(10);
        $stats = [
            'roles' => Role::count(),
    ...
    {
        $role->load(['permissions' => fn ($query) => $query->orderBy('group_name')->orderBy('display_name'), 'users']);
    ...
    {
        if ($role->is_system || in_array($role->name, ['super-admin', 'admin'], true)) {
    ...
    private function validateRole(Request $request, ?Role $role = null): array
    {
        return $request->validate([
            'name' => [$role ? 'sometimes' : 'required', 'string', 'max:80', Rule::unique('roles', 'name')->ignore($role?->id)],
